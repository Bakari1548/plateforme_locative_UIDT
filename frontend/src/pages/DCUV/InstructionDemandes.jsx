...
export default function InstructionDemandes() {
  ...
  async function loadDemandes() {
    ...
    if (result.error) {
      ...
      if (filter === 'all') setAllDemandes(result.demandes || [])
    }
    setLoading(false)
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold text-accent-dark mb-1">Instruction des demandes</h1>
      <p className="text-sm text-accent-slate mb-6">Demandes à instruire</p>
        ...
        {loading ? (
          ...
          <div className="bg-white shadow-sm rounded-lg p-8 text-center">
            <ClipboardList className="h-12 w-12 text-accent-light mx-auto mb-3" />
            <p className="text-accent-slate">Aucune demande à instruire</p>
          </div>
        ) : (
          <div className="space-y-4">
            {demandes.map((demande) => {
              ...
              return <div key={demande.id} className="bg-white shadow-sm rounded-lg p-6">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-accent-light">
              <h2 className="text-lg font-semibold">Instruction: {selectedDemande.numero_suivi}</h2>
            </div>
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => handleStatutChange(selectedDemande.id, 'en_instruction')}
                ...
                >
                  Incomplet (compléments)
                </button>
                <button
                  onClick={() => handleStatutChange(selectedDemande.id, 'rejete')}
                  disabled={actionLoading}
                ...
                <button
                  onClick={() => handleStatutChange(selectedDemande.id, 'en_commission')}
                  disabled={actionLoading}
